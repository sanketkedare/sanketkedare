  ...
  return (
    ...
    >
      ...
      <div className="w-full lg:w-[80%] mx-auto px-4 sm:px-6 lg:px-0 relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-full bg-purple-100/90 dark:bg-purple-500/10 border border-purple-300/80 dark:border-purple-500/20 text-purple-900 dark:text-purple-400 text-[11px] sm:text-xs font-black uppercase tracking-wider mb-2.5 shadow-2xs">
        ...
        {/* ── 2. Core Identity Banner (Open Border Accent & Soft Shadow) ──── */}
        <motion.div
        ...
        >
          ...
          <div className="p-2.5 sm:p-4 bg-slate-100/60 dark:bg-white/[0.02] border-b border-slate-200/80 dark:border-white/10">
            <div className="flex items-center justify-between mb-2.5 px-1">
              <div className="flex items-center gap-2">
                <FiSliders size={13} className="text-cyan-700 dark:text-cyan-400" />
              ...
              <span className="text-[10px] sm:text-[11px] text-slate-600 dark:text-slate-400 font-medium hidden xs:inline-block">
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-between px-2.5 py-2 sm:px-3 sm:py-2.5 rounded-xl border text-left transition-all duration-200 cursor-pointer ${
                      isSelected
                        ? 'bg-gradient-to-r from-cyan-500/20 to-purple-500/20 border-cyan-500/60 text-cyan-950 dark:text-cyan-300 shadow-xs'
                        : 'bg-white/60 dark:bg-white/[0.03] border-slate-200/80 dark:border-white/5 text-slate-800 dark:text-slate-400 hover:border-purple-300 dark:hover:border-white/20'
                    }`}
                  >
                    <span className="text-[11px] sm:text-xs md:text-sm font-black truncate mr-1.5">
                      {item.axis}
                    </span>
                    <span
                      className={`text-[10px] sm:text-[11px] font-black px-1.5 py-0.5 rounded-md shrink-0 ${
                        isSelected
                          ...
                          : 'bg-slate-200/80 dark:bg-white/5 text-slate-700 dark:text-slate-500'
                      }`}
                    >
                      {item.score}%
                    </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 md:gap-8 p-4 sm:p-6 md:p-8 items-stretch">
            ...
            <div className="lg:col-span-5 flex flex-col items-center justify-between gap-4 sm:gap-5 p-3 sm:p-4 rounded-2xl bg-white/40 dark:bg-[#070817]/90 border border-slate-200/70 dark:border-white/5 shadow-xs">
              <div className="w-full flex items-center justify-between">
                <span className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-cyan-900 dark:text-cyan-400">
                  Tradeoff Vector Plane
                </span>
                <span className="text-[11px] sm:text-xs font-black px-2 py-0.5 rounded-md bg-purple-100 dark:bg-purple-500/10 text-purple-900 dark:text-purple-400 border border-purple-300 dark:border-purple-500/20 truncate max-w-[150px]">
                  {activeTradeoff.axis}
                </span>
              </div>
              ...
              <div className="w-full bg-slate-100/70 dark:bg-white/[0.02] p-3 sm:p-3.5 rounded-xl border border-slate-200/80 dark:border-white/5">
                <div className="flex items-center justify-between text-[11px] sm:text-xs mb-1.5 gap-2">
                  <span className="font-black text-cyan-900 dark:text-cyan-400 truncate">
                    {activeTradeoff.left_anchor}
                  </span>
                  <span className="font-bold text-slate-700 dark:text-slate-400 truncate text-right">
            ...
            <div className="lg:col-span-7 flex flex-col justify-between gap-4 sm:gap-5">
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-3 sm:gap-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[11px] sm:text-xs font-black text-cyan-900 dark:text-cyan-400 bg-cyan-100 dark:bg-cyan-500/10 border border-cyan-300 dark:border-cyan-500/20 px-2 py-0.5 rounded-md">
                          {activeRule.id}
                        </span>
                    ...
                    <div className="text-right shrink-0">
                      <div className="text-xl sm:text-2xl md:text-3xl font-black text-cyan-800 dark:text-cyan-400 leading-none">
                        {activeTradeoff.score}%
                      </div>
                      <span className="text-[9px] sm:text-[10px] uppercase tracking-wider font-black text-slate-600 dark:text-slate-400 block mt-0.5">
                        Rigidity Score
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="p-3 sm:p-3.5 rounded-xl bg-purple-100/60 dark:bg-white/[0.03] border-l-4 border-cyan-500 text-xs sm:text-sm text-slate-900 dark:text-slate-300 leading-relaxed font-bold">
                    {activeRule.statement}
                  </div>

                  {/* Rationale Brief */}
                  <p className="text-xs sm:text-sm text-slate-800 dark:text-slate-400 font-medium leading-relaxed">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 mt-1">
                    {/* Rejected Alternative (Left Border Accent & Soft Shadow) */}
                    <div className="p-3.5 sm:p-4 rounded-xl border-l-4 border-l-rose-500 bg-white/70 dark:bg-rose-500/10 border border-slate-200/80 dark:border-rose-500/20 shadow-xs flex flex-col justify-between">
                      <div>
                        <div className="flex items-center gap-1.5 text-rose-700 dark:text-rose-400 mb-2">
                          <FiXCircle size={15} />
                        ...
                        <p
                          className="text-xs sm:text-sm text-slate-900 dark:text-rose-100/80 font-semibold leading-relaxed"
                          style={{ fontFamily: 'Cambria, Cochin, Georgia, Times, serif' }}
                        >
                          {activeLog.rejected_alternative}
                        </p>
                      </div>
                      <span className="text-[10px] font-black text-rose-700 dark:text-rose-400/70 mt-2.5 block">
                        ✕ Avoided Temporary Band-Aid
                      </span>
                    </div>

                    {/* Engineered Solution (Left Border Accent & Soft Shadow) */}
                    <div className="p-3.5 sm:p-4 rounded-xl border-l-4 border-l-emerald-500 bg-white/70 dark:bg-emerald-500/10 border border-slate-200/80 dark:border-emerald-500/20 shadow-xs flex flex-col justify-between">
                      <div>
                        <div className="flex items-center gap-1.5 text-emerald-800 dark:text-emerald-400 mb-2">
                        ...
                        <p
                          className="text-xs sm:text-sm text-slate-900 dark:text-emerald-100/90 leading-relaxed font-semibold"
                          style={{ fontFamily: 'Cambria, Cochin, Georgia, Times, serif' }}
                        >
                          {activeLog.chosen_solution}
                        </p>
                      </div>
                      <span className="text-[10px] font-black text-emerald-800 dark:text-emerald-400/80 mt-2.5 block">
                  ...
                  <div className="p-2.5 sm:p-3 rounded-xl bg-purple-100/90 dark:bg-purple-500/10 border border-purple-300 dark:border-purple-500/20 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1 sm:gap-2 text-xs text-purple-950 dark:text-purple-300">
                    <span className="font-black shrink-0">Production Impact:</span>
                    <span className="text-left sm:text-right italic font-semibold">
        ...
        {/* ── 4. Core Mental Models (Open Flow Layout with Left Border Accents & Shadows) ─ */}
        <motion.div
          initial={{ opacity: 0, y: 18 }}
          animate={isInView ? { opacity: 1, y: 0 } : {}}
          transition={{ duration: 0.6, delay: 0.3 }}
          className="pt-6 border-t border-slate-200/80 dark:border-white/10"
        >
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-1 sm:gap-2 mb-6">
            <div>
              <span className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-purple-900 dark:text-purple-400 block mb-1">
                ⚡ Foundational Reasoning
              </span>
              <h3
                className="text-lg md:text-3xl font-black text-slate-900 dark:text-white tracking-tight"
                style={{ fontFamily: 'Cambria, Cochin, Georgia, Times, serif' }}
              >
                Core Mental Models
              </h3>
            </div>
            <span className="text-xs text-slate-700 dark:text-slate-400 font-bold hidden sm:inline-block">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
            {mental_models.map((model, idx) => {
              ...
              return (
                ...
                >
                  <div>
                    <div className="text-2xl mb-3">{MODEL_ICONS[idx]}</div>
                    <h4
